...
public class FadeObjectBlockingObject : MonoBehaviour
{
    ...
    private IEnumerator FadeObjectIn(FadingObject FadingObject)
    {
        ...
        for (int i = 0; i < FadingObject.Materials.Count; i++)
        {
            ...
            FadingObject.Materials[i].SetInt("_SrcBlend", (int)UnityEngine.Rendering.BlendMode.One); // affects both "Transparent" and "Fade" options
            FadingObject.Materials[i].SetInt("_DstBlend", (int)UnityEngine.Rendering.BlendMode.Zero); //affects both "Transparent" and "Fade" options
            FadingObject.Materials[i].SetInt("_ZWrite", 1); //Disable Z writing
